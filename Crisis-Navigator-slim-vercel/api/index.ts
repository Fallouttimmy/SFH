...
function getHelplinesByCategory(categoryId: string): Helpline[] {
  return helplines.filter((h) => h.categoryId === categoryId);
}

function getFeaturedHelplines(): Helpline[] {
  return helplines.filter((h) => h.isFeatured);
}

function searchHelplines(query: string): Helpline[] {
...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // GET /api/helplines
    if (path === "/helplines" || path === "/helplines/") {
      return res.status(200).json(helplines);
    }
    
    // GET /api/helplines/featured
    if (path === "/helplines/featured" || path === "/helplines/featured/") {
      return res.status(200).json(getFeaturedHelplines());
    }
    
    // GET /api/helplines/category/:slug
    const helplinesCategoryMatch = path.match(/^\/helplines\/category\/([^/]+)$/);
    if (helplinesCategoryMatch) {
      const slug = helplinesCategoryMatch[1];
      const category = getCategoryBySlug(slug);
      if (!category) {
        return res.status(404).json({ error: "Category not found" });
      }
      return res.status(200).json(getHelplinesByCategory(category.id));
    }
    
    // GET /api/helplines/search/:query
    const searchMatch = path.match(/^\/helplines\/search\/([^/]+)$/);
    if (searchMatch) {
      const query = decodeURIComponent(searchMatch[1]);
      return res.status(200).json(searchHelplines(query));
    }
